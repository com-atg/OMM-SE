        ...
        <section class="bg-white rounded-xl shadow-sm ring-1 ring-slate-200 p-6">
            <form method="POST" action="{{ route('admin.users.update', $user) }}" class="space-y-5">
                ...
                <div>
                    ...
                    <div class="text-sm text-slate-800 bg-slate-50 rounded-md px-3 py-2 ring-1 ring-slate-200">{{ $user->email }}</div>
                    <p class="mt-1 text-xs text-slate-500">Email is set by Okta SAML and cannot be changed here.</p>
                </div>

                <div>
                    <label for="name" class="block text-xs uppercase tracking-wide text-slate-500 mb-1">Name</label>
                ...
                <div>
                    ...
                    <input type="text" name="redcap_record_id" id="redcap_record_id" value="{{ old('redcap_record_id', $user->redcap_record_id) }}"
                        class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-200 focus:outline-none">
                ...
                <div class="pt-2 flex justify-end gap-3">
                    <a href="{{ route('admin.users.index') }}" class="rounded-lg px-4 py-2 text-sm font-medium text-slate-600 hover:bg-slate-100">Cancel</a>
