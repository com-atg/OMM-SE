...
<script>
    ...
    renderScholarCharts(document);

    function registerScholarChartHook() {
        if (window.scholarDetailChartsHooked || ! window.Livewire) {
            return;
        }

        window.scholarDetailChartsHooked = true;
    ...
    if (window.Livewire) {
        registerScholarChartHook();
    } else {
        document.addEventListener('livewire:init', registerScholarChartHook, { once: true });
    }
</script>
